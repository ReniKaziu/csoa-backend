...
export class ReviewController {
  static listOppositeTeamPlayers = async (request: Request, response: Response) => {
    try {
      const event = await EventService.findById(+request.params.eventId, false);
      if (Helper.isDefined(event)) {
        const results = await ReviewService.listOppositeTeamPlayers(event, request, response);
