...
import { ERROR_CODES, ERROR_MESSAGES } from "../../common/utilities/ErrorMessages";
...
export class UserMiddleware {
  ...
  static validationPasswordInput = (request: Request, response: Response, next: NextFunction) => {
    const registerInput = Joi.object().keys({
    ...
    if (!Helper.isDefined(result.error)) {
      ...
      return response
        ...
        .send(new ErrorResponse(ERROR_MESSAGES.VALIDATION_ERROR, result.error.details));
    }
  };
}
