...
export class NotificationService {
  ...
  static storeNotification = async (payload) => {
    ...
    if (payload[0]?.type === NotificationType.CHAT_USER) {
      ...
      const foundMessage = await notificationRepository
        ...
        .getOne();
      if (foundMessage) {
        notificationRepository.merge(foundMessage, payload[0]);
        await notificationRepository.save(foundMessage);
    ...
    if (payload[0]?.type === NotificationType.CHAT_TEAM) {
      const foundTeamMessage = await notificationRepository
        ...
        .getOne();
      if (foundTeamMessage) {
        notificationRepository.merge(foundTeamMessage, payload[0]);
        await notificationRepository.save(foundTeamMessage);
    ...
    if (payload[0]?.type === NotificationType.CHAT_EVENT) {
      ...
      if (foundEventMessage) {
        notificationRepository.merge(foundEventMessage, payload[0]);
        await notificationRepository.save(foundEventMessage);
        return "Notification succesfully updated";
      }
    }
  ...
  static createRequestNotification = async (
    ...
    const titles = {
      ...
      [NotificationType.CREATOR_EXCLUDED_FROM_EVENT]: `Krijuesi i eventit ${senderName} ju largoi nga eventi ${eventName} per ${sportsMapped[sport]}`,
